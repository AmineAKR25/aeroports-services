...
export default function SiteShell() {
  ...
  const [coverageDirectoryOpen, setCoverageDirectoryOpen] = useState(false)
    ...
    <header className={`site-header${headerScrolled ? ' is-scrolled' : ''}`}>
      <div className="shell header-row">
        ...
        <div className="header-actions">
          <a className="ops-link" href="tel:+33660475916"><span className="ops-link-copy"><small>OPS · 24h/24 · 7j/7</small><strong>+33 (6) 60 47 59 16</strong></span><Icon name="phone" size={18} /></a>
    ...
    <main id="main-content">
        ...
        <div className="shell hero-grid">
          <div className="hero-copy">
            ...
            <div className="hero-proof">
              ...
              <div className="hero-proof-availability"><span><strong>Disponible 24h/24 · 7j/7</strong><small>Toute l’année</small></span></div>
            </div>
      ...
      <section aria-labelledby="coverage-title" className="section coverage-section" data-section="coverage" id="coverage"><div className="shell"><SectionIntro copy="Recherchez les implantations publiées. La carte apporte le contexte géographique ; le répertoire reste la référence complète et accessible." eyebrow="Implantations publiées" id="coverage-title" title="Un réseau national, lieu par lieu." />
        <div className="coverage-explorer">
          <div className="coverage-search-block"><label className="coverage-search"><span>Rechercher un lieu</span><span className="search-control"><Icon name="search" size={19} /><input aria-label="Rechercher un lieu" type="search" value={coverageSearch} onChange={(event) => setCoverageSearch(event.target.value)} placeholder="Ex. Orly, Gare de Lyon…" /></span></label></div>
          <div className="network-map"><div className="map-toolbar"><div><span>Vue géographique</span><strong>{categoryLabels[coverageCategory]}</strong></div><div className="map-active"><span className={`category-shape category-shape--${activeLocation.category}`} /><span><small>Sélection</small><strong>{activeLocation.name}</strong></span></div></div><RealMap highlightedLocationId={highlightedLocation} locations={locations} onHighlightLocation={setHighlightedLocation} onSelectLocation={chooseLocation} selectedLocationId={selectedLocation} /><p className="map-note">Les coordonnées disposent d’un statut de provenance ; les repères provisoires restent à valider par l’équipe d’exploitation avant mise en production.</p></div>
          <div className="coverage-directory">
            <button aria-controls="coverage-directory-panel" aria-expanded={coverageDirectoryOpen} className="coverage-directory-toggle" onClick={() => setCoverageDirectoryOpen((current) => !current)} type="button"><span>{coverageDirectoryOpen ? 'Masquer' : 'Afficher'} les filtres et les lieux</span><Icon name="chevron" size={18} /></button>
            <div className={`coverage-directory-panel${coverageDirectoryOpen ? ' is-open' : ''}`} hidden={!wideViewport && !coverageDirectoryOpen} id="coverage-directory-panel">
